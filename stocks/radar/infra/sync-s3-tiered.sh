...
aws s3 sync "$DIST" "s3://$BUCKET" "${PROFILE_ARGS[@]}" \
  ...
  "${EXCLUDE_STATE[@]}" \
  --exclude "_astro/*" \
  --exclude "quotes.json" --exclude "screener.json" --exclude "build-meta.json" --exclude "health.json" --exclude "settings.json" \
...
aws s3 sync "$DIST" "s3://$BUCKET" "${PROFILE_ARGS[@]}" \
  "${EXCLUDE_STATE[@]}" \
  --exclude "*" \
  --include "quotes.json" --include "screener.json" --include "datacenter/news.json" \
