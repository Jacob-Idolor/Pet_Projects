...
const SCRIPT = resolve(ROOT, "scripts/fetch/fetch-screener.py");
...
if (process.env.SCREENER_SKIP === "1") {
  ...
  if (existsSync(OUT)) {
    spawnSync(process.execPath, [resolve(ROOT, "scripts/fetch/write-dc-movers.mjs")], {
      cwd: ROOT,
      encoding: "utf8",
      env: process.env,
      stdio: "inherit",
...
if (result.status === 0) {
  ...
  const movers = spawnSync(process.execPath, [resolve(ROOT, "scripts/fetch/write-dc-movers.mjs")], {
    cwd: ROOT,
    encoding: "utf8",
    env: process.env,
    stdio: "inherit",
