...
async function watchlistMood() {
  if (!site) return { ok: false, note: "No site URL set — skip mood" };
  try {
    const res = await fetch(`${site}/quotes.json`, { cache: "no-store" });
    if (!res.ok) return { ok: false, note: `quotes.json HTTP ${res.status}` };
    ...
    if (!n) return { ok: false, note: "No quote changes yet" };
    const avg = sum / n;
    let mood = "mixed";
