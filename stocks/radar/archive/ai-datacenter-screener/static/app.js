...
function scoreBreakdown(h) {
  ...
  const setup = p.setup ?? null, trigger = p.trigger ?? null;
  let signalHtml = "";
  if (setup != null && trigger != null) {
    const sig = actionSignal(setup, trigger);
    signalHtml = `<div class="sb-signal">
      <span class="sb-st">Setup <b style="color:${scoreColor(setup)}">${setup}</b></span>
      <span class="sb-st-sep">·</span>
      <span class="sb-st">Trigger <b style="color:${scoreColor(trigger)}">${trigger}</b></span>
      <span class="sb-action ${sig.cls}">${sig.label}</span>
      <span class="sb-action-note">${sig.note}</span></div>`;
  }
  return `<div class="sb">
