...
if "data" in data and isinstance(data["data"], list):
    ...
    # Print all projection information to a text file
    with open("projection_info.txt", "w", encoding="utf-8") as file:
        table_rows = []
        for projection_info_entry in projection_info:
            table_rows.append([f"{key}: {value}" for key, value in projection_info_entry.items()])
        file.write(tabulate(table_rows, headers="keys", tablefmt="fancy_grid", stralign="left", numalign="right"))
