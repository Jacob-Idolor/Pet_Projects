...
export const PROGRESS_KEY = "tf-lab-progress-v1";
...
export function syncAutoCompletions(data: ProgressData): ProgressData {
  ...
  for (const mod of modules) {
    ...
    if (localStorage.getItem(`tf-quiz-${mod.id}`) === "passed" && !next.completed[labId]) {
      next.completed[labId] = { completedAt: new Date().toISOString(), note: "Auto: quiz passed" };
      changed = true;
    }
  }

  const allQuizzesPassed = modules.every((m) => localStorage.getItem(`tf-quiz-${m.id}`) === "passed");
  ...
  const tfMissions = JSON.parse(localStorage.getItem("tf-missions") || "[]") as string[];
  if (tfMissions.length >= 6 && !next.completed["extra-tf-missions"]) {
    next.completed["extra-tf-missions"] = { completedAt: new Date().toISOString(), note: "Auto: all simulator missions" };
    changed = true;
  }
